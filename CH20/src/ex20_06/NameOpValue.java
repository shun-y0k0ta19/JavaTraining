	...
	public void calc(Reader source) throws IOException {
		...
		while (in.nextToken() != StreamTokenizer.TT_EOF) {
			if (in.sval.equals("One")) {
				...
				throw new IllegalArgumentException(" One or Two or Three");
			}
			in.wordChars('+', '+');
			in.ordinaryChar('-');
			in.wordChars('-', '-');
			...
			if (in.sval.equals("+")) {
				...
				throw new IllegalArgumentException("Option: + or - or =");
			}

			in.parseNumbers();
			if (in.nextToken() != StreamTokenizer.TT_NUMBER) {
				throw new IllegalArgumentException("Value: only number");
			}
